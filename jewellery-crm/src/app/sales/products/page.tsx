...
import { Search, Plus, Filter, MoreHorizontal, Package, Tag, TrendingUp, Eye, Edit, Trash2, X, Store, Globe, ShoppingCart, Star } from 'lucide-react';
...
interface Product {
  ...
  created_at?: string;
  updated_at?: string;
  // Additional properties for sales view
  cost_price?: number;
...
export default function SalesProductsPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      // For sales users, use the "my" endpoint to get products they can access
  ...
  const handleAddToCart = (product: Product) => {
    ...
    // Here you would typically add the product to a cart state or send to backend
  };
  ...
  // Calculate stats for sales users
  const totalProducts = products.length;
  const activeProducts = products.filter(p => p.status === 'active').length;
  const lowStockProducts = products.filter(p => (p.quantity ?? p.stock_quantity ?? 0) <= 3).length;
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Browse and manage your product catalog for sales
          </p>
        </div>
